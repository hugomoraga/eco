...
from core.application.processors.damage import calculate_damage
...
def handle_npc_damage_to_player(
    ...
    if damage > 0:
        ...
        if person.vitality <= 0:
            log.error(
                "player_death", turn=turn, player=person.name, vitality=0, trigger="npc_damage"
            )
...
def trigger_player_death(
    ...
    log.debug(
        "reincarnation",
        stage="preserving_legacy",
        player_person_id=player_person.id,
        echo_id=player_person.echo_id,
    )
    ...
    if not echo:
        log.error(
            ...
            stage="trigger_death",
            error="no_echo_found",
            player_person_echo_id=player_person.echo_id,
        )
        return
    ...
    if new_person:
        ...
        log.info(
            ...
            stage="success",
            echo_name=echo.name,
            old_person=player_person.name,
            new_person=new_person.name,
            transition_turn=world.transition_turn,
        )
...
def handle_reincarnation(
    ...
    if not person:
        log.warning(
            "reincarnation", stage="end_transition", error="no_active_player_person", turn=turn
        )
        return
...
def check_and_handle_reincarnation(
    world: World, notify, turn: int, log: Logger | None = None
) -> bool:
    """
