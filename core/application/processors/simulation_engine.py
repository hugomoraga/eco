...
from core.application.processors.goal_processor import evaluate_goals, initialize_goals
from core.application.processors.npc_processor import process_npc_turns
from core.application.processors.observer import NullObserver, SimulationObserver
from core.application.processors.random import SeededRandom
from core.application.processors.simulation_api import SimulationAPI
from core.application.processors.turn_context import (
...
class SimulationEngine:
    ...
    def __init__(
        ...
        if player is not None:
            ...
            self._player: Player = AutoPlayer(seed=seed)
    ...
    def _save_snapshot(self, world: World, turn: int) -> str | None:
        try:
    ...
    def run(self) -> dict:
        ...
        event_gen = EventGenerator(ai_adapter, pool=event_pool, seed=self.seed, log=self._log)
        event_interval = 5
        ...
        try:
            ...
            self._log.error(
                "finale_generation_failed",
                turn=self.turn,
                error=str(e),
                error_type=type(e).__name__,
            )
            finale_data = {"error": str(e), "turn": self.turn}
    ...
    def _execute_turn(self, npc_executor, ai_adapter, faction_system, event_gen, event_interval, faction_tick_interval):
        ...
        check_and_handle_reincarnation(self.world, self._notify, self.turn, self._log)
