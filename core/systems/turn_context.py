...
from core.domain.person import PlayerPerson
from core.domain.world import World
from core.systems.damage import calculate_damage, should_deal_damage_to_player
from core.systems.reincarnation import (
    end_transition_turn,
    is_in_transition,
...
def handle_npc_damage_to_player(
    ...
    if damage > 0:
        ...
        if person.vitality <= 0:
            ...
            trigger_player_death(world, person, notify, turn)
...
def check_and_handle_reincarnation(world: World, notify, turn: int) -> bool:
    """
    ...
    if is_in_transition(world):
        if world.transition_turn <= world.clock.action_tick:
            handle_reincarnation(world, notify, turn)
