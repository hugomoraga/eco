...
def process_npc_turns(world, seed: int, turn: int, npc_executor, ai_adapter, notify, log, log_event) -> None:
    """
    Process NPC turns at start of each turn (before player action).
    """
    from adapters.autoplayer import NPCEngine, process_npc_turn
    ...
    if npc_actions_log:
        ...
        top_actions = sorted(npc_actions_log, key=lambda a: a["influence"], reverse=True)[:top_n]
        for action in top_actions:
            notify("on_npc_action", turn, action["npc_name"], action["action"], action["message"])
